...
WTWJS.prototype.globalLogin = async function(zparameters) {
	/* references 3dnet.roomz.com - global roomz login complete and confirms the local values from login */
	try {
		let zglobaluserid = '';
		let zdisplayname = '';
		let zusertoken = '';
...
WTWJS.prototype.globalLoginResponse = function(zresponse) {
	/* references 3dnet.roomz.com - global roomz login complete and returns the local values from login */
	let znoerror = true;
	try {
		var zerror = '';
		if (zresponse != null) {
		...
		switch (zitem) {
			...
			case '3D Website Login':
				zpagediv += "<h2 class='wtw-login'>3D Website Login</h2>" +
					...
					"<div class='wtw-loginbutton' onclick='WTW.loginAttempt();'><img src='/content/system/images/icon-128x128.jpg' alt='Bixma' title='Bixma' class='wtw-image40'/><div style='margin-top:10px;'>3D Website Login</div></div>" +
			...
			case 'Email Verification':
				zpagediv += "<h2 class='wtw-login'>Email Verification</h2>" +
					...
					"<div class='wtw-loginbutton' onclick=\"WTW.checkEmailValidation('');\"><img src='/content/system/images/wtwlogo.png' alt='Bixma' title='Bixma' class='wtw-image40'/><div style='margin-top:10px;'>Continue</div></div>" +
