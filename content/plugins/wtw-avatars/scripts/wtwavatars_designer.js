...
WTWJS.prototype.createScene = function() {
	/* create the avatar designer scene - lighting designed to match the default roomz scene */
	try {
		console.log('%c\r\n\r\nroomz 3D Avatar Designer\r\nBixma (v1.0.1) 08-29-2022', 'color:green;font-weight:bold;');
		...
		switch (zactive) {
				...
				if (dGet('wtw_tavatarid').value != '') {
					...
					if (wtw_showglobal) {
						...
						if (dGet('wtw_tglobaluserid').value != '') {
							...
							ztext2.text = 'Works on most roomz 3D Websites Globally';
							ztext2.color = '#afafaf';
						} else {
						...
						var zlabel1 = BABYLON.GUI.Control.AddHeader(zbutton1, 'roomz Global Avatar', '270px', { isHorizontal: true, controlFirst: true });
						zlabel1.height = '40px';
						zlabel1.width = '270px';
						zlabel1.color = 'white';
			...
			case 4:   /* save avatar settings */
				if (dGet('wtw_tavatarid').value != '') {
					...
					let zsaved = 'roomz Global and the Local 3D Websites';
...
WTWJS.prototype.loadDesignerAvatar = async function(zglobaluseravatarid, zuseravatarid) {
	...
	try {
		WTW.startLoading();
		/* if it is a global avatar get form 3dnet.roomz.com */
			...
			WTW.postAsyncJSON('https://3dnet.roomz.com/connect/globalavatar.php', zrequest, 
				async function(zresponse) {
					...
					if (zresponse.avatar != null) {
						...
						WTW.postAsyncJSON('https://3dnet.roomz.com/connect/globalavatars.php', zrequest2, 
							function(zresponse2) {
								if (zresponse2 != null) {
			...
			async function(zresponse) {
				...
				if (zresponse.useravatarid != '') {
					...
					if (dGet('wtw_tglobalavatar').value == '1') {
						/* if checkbox for global is checked, save to global 3dnet.roomz.com */
						var zsecureprotocol = '0';
						if (wtw_protocol == 'https://') {
							zsecureprotocol = '1';
						}
						if (zresponse.objectfolder.indexOf('http') == -1) {
							zresponse.objectfolder = 'https://3dnet.roomz.com' + zresponse.objectfolder;
						}
						var zrequest2 = {
							'usertoken':dGet('wtw_tusertoken').value,
							'globaluserid':btoa(dGet('wtw_tglobaluserid').value),
