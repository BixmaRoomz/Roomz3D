...
class wtwcoins_functions {
	...
	private function collectCoinGlobal($zwebid, $zactionzoneid, $zvalue1, $zuserid, $zglobaluserid, $zusertoken) {
		...
		try {
			...
			/* it will validate the currently logged in account on the roomz servers */
			if ($wtwplugins->hasValue($zglobaluserid) && $wtwplugins->hasValue($zusertoken)) {
				...
				$zresponse = $wtwplugins->openFilefromURL('https://3dnet.roomz.com/connect/wtwcoins.php', false, $zrequest);
			}
		} catch (Exception $e) {
