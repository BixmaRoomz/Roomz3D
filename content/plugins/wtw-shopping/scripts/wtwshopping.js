...
//		var zstore = WTWShopping.getStoreData(zmoldname);
		...
		if (zmoldsloaded == 0) {
			/* mold-store settings have not been fetched - so fetch them from local roomz site */
				...
				WTW.getAsyncJSON(zurl, 
					function(zresponse) {
						if (zresponse != null) {
							...
							WTWShopping.productLoadProducts(zmoldname, zcategoryid, JSON.parse(zresponse), zstoreinfo.storeurl, 'roomz');
						}
					}
				);
			} 
...
wtwshopping.prototype.loadProductDisplay = async function(zmoldname, zproductname, zprice, zproductid, zslug, zimageurl, zshortdescription, zdescription) {
	try {
		...
		if (zstoreinfo.storeurl != '') {
			...
			if (zimageurl != '') {
				WTW.getAsyncJSON(zstoreinfo.storeurl + '/roomz/image.php?roomz_image_url=' + zimageurl.replace('?','&'), 
					function(zresponse2) {
						if (zresponse2 != null) {
...
wtwshopping.prototype.getStoreInfo = async function(zmoldname) {
	try {
		...
		if (zstoreinfo.woocommerceapiurl != '' && zwebname == '') {
			var zurl = zstoreinfo.storeurl + '/roomz/storeinfo.php?roomz_store_info=1';
			WTW.getAsyncJSON(zurl, 
				function(zresponse) {
			...
			WTW.getAsyncJSON(zurl, 
				function(zresponse) {
					if (zresponse != null) {
						/* process results */
						WTWShopping.productClearFetchProducts(zmoldname);
						WTWShopping.productLoadProducts(zmoldname, '', JSON.parse(zresponse), zstoreinfo.storeurl, 'roomz');
					}
				}
			);
		} 
