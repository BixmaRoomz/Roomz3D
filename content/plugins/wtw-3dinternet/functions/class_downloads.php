...
class wtw_3dinternet_downloads {
	...
	public function downloadWeb($zwebid, $znewwebid, $zwebtype, $zusertoken, $zdownloadparentwebid, $zdownloadparentwebtype, $zcommunityid, $zbuildingpositionx = 0, $zbuildingpositiony = 0, $zbuildingpositionz = 0, $zbuildingscalingx = 1, $zbuildingscalingy = 1, $zbuildingscalingz = 1, $zbuildingrotationx = 0, $zbuildingrotationy = 0, $zbuildingrotationz = 0) {
		/* this process downloads 3D Web and dependent objects from https://3dnet.roomz.com (roomz repository)*/
		...
		try {
			...
			if ($wtwhandlers->hasValue($zuserid)) {
				...
				$zurl = "https://3dnet.roomz.com/connect/sharedownload.php?webid=".$zwebid."&webtype=".$zwebtype."&userid=".$zuserid."&serverinstanceid=".$wtwhandlers->serverinstanceid."&domainurl=".$wtwhandlers->domainurl;
	...
	public function downloadUpdateWeb($zwebid, $zupdatewebid, $zwebtype, $zusertoken) {
		/* this process downloads updated version of 3D Web from https://3dnet.roomz.com (roomz repository)*/
...
//				$zwebid = $wtwhandlers->getNewKey($zwebtypes, $zwebtype."id", $zwebid);
				...
				/* fetch the 3D Web data structure for the repository (all of the associated records) */
				$zurl = "https://3dnet.roomz.com/connect/sharedownload.php?webid=".$zupdatewebid."&webtype=".$zwebtype."&userid=".$zuserid."&serverinstanceid=".$wtwhandlers->serverinstanceid."&domainurl=".$wtwhandlers->domainurl;
	...
	public function downloadAvatar($zwebid, $znewwebid, $zusertoken) {
		...
		try {
			...
			if ($wtwhandlers->hasValue($zuserid)) {
				...
				/* fetch the 3D Web data structure for the repository (all of the associated records) */
				$zurl = "https://3dnet.roomz.com/connect/sharedownload.php?webid=".$zwebid."&webtype=avatar&userid=".$zuserid."&serverinstanceid=".$wtwhandlers->serverinstanceid."&domainurl=".$wtwhandlers->domainurl;
		...
		try {
			...
			foreach ($zrequestfiles as $zfile) {
				if (strpos($zfile->file,'/') !== false) {
					...
					$wtwhandlers->getFilefromURL('https://3dnet.roomz.com/content/uploads/avatars/'.$zwebid.'/'.$zfolder.'/'.$zfilename, wtw_rootpath.$zobjectfolder.$zfolder.'/', $zfilename);
				} else {
					$wtwhandlers->getFilefromURL('https://3dnet.roomz.com/content/uploads/avatars/'.$zwebid.'/'.$zfile->file, wtw_rootpath.$zobjectfolder, $zfile->file);
				}
			}
		} catch (Exception $e) {
			...
			if ($wtwhandlers->hasValue($zuserid)) {
				...
				foreach ($zrequest->files as $zfile) {
					if (strpos($zfile->file,'/') !== false) {
						...
						$wtwhandlers->getFilefromURL('https://3dnet.roomz.com/content/uploads/avatars/'.$zupdatewebid.'/'.$zfolder.'/'.$zfilename, wtw_rootpath.$zobjectfolder.$zfolder.'/', $zfilename);
					} else {
						$wtwhandlers->getFilefromURL('https://3dnet.roomz.com/content/uploads/avatars/'.$zupdatewebid.'/'.$zfile->file, wtw_rootpath.$zobjectfolder, $zfile->file);
					}
				}
