...
function WTW_3DINTERNET() {
	...
	this.masterDownloads = '1'; /* toggle off or on roomz Downloads for 3D Webs - 3D Communities, 3D Buildings, 3D Things, and 3D Avatars (server level) */
	this.masterPlugins = '1'; /* toggle off or on roomz Downloads for 3D Plugins (server level) */
...
WTW_3DINTERNET.prototype.adminLoadAfterScreen = function(zhmenu) {
	...
	try {
		/* check for updates to roomz and 3D Plugins */
...
WTW_3DINTERNET.prototype.openFullPageForm = function(zpageid, zsetcategory, zitem, zitemname, zitemnamepath, zpreviewname) {
	...
	try {
		...
		switch (zpageid) {
			...
			case 'importpage':
				if (WTW.adminView == 1) {
					dGet('wtw_fullpageformtitle').innerHTML = "<div class='wtw-toparrowlink' onclick=\"dGet('wtw_modelfilter').value='';dGet('wtw_tgroupuploadobjectid').value='';dGet('wtw_tgroupdiv').value='';WTW.openFullPageForm('medialibrary','');WTW.setImageMenu(4);\">" + WTW.__('Media Library') + "</div><img id='wtw_arrowicon1' src='/content/system/images/menuarrow32.png' alt='' title='' class='wtw-toparrowicon' /><div class='wtw-toparrowtext'>" + WTW.__('roomz Downloads') + "</div>";
...
WTW_3DINTERNET.prototype.openDashboardForm = async function(zshow) {
	...
	try {
		if (zshow) {
			...
			WTW.getAsyncJSON('https://3dnet.roomz.com/connect/videolinks.php', 
				function(zresponse) {
			...
			WTW.getAsyncJSON('https://3dnet.roomz.com/connect/wtwactivities.php', 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					var zwtwactivities = '';
					for (var i=0;i<zresponse.length;i++) {
							...
							switch (zresponse[i].category) {
								...
								case 'roomz Servers':
									if (zresponse[i].sharedimage == '') {
										zresponse[i].sharedimage = 'https://3dnet.roomz.com/wp-content/uploads/2021/02/NewWTWServer.png';
									}
...
WTW_3DINTERNET.prototype.responseLoadLoginSettings = async function(zsettings, zparameters) {
	...
	try {
		...
		WTW.getAsyncJSON('https://3dnet.roomz.com/connect/myservices.php?serverinstanceid=' + dGet('wtw_serverinstanceid').value + '&serverip=' + dGet('wtw_serverip').value, 
			function(zresponse) {
				zresponse = JSON.parse(zresponse);
		...
		if (WTW.adminView == 1) {
			if (dGet('wtw3dinternet_enableglobaltext') != null) {
				...
				if (wtw3dinternet.masterDownloads == '1') {
					...
					dGet('wtw3dinternet_enabledownloadstext').innerHTML = 'roomz Downloads Enabled';
					...
					dGet('wtw3dinternet_enabledownloadstext').innerHTML = 'roomz Downloads Disabled';
...
WTW_3DINTERNET.prototype.serviceCheck = async function(zservice) {
	/* check for multiplayer services on main roomz hub */
	try {
		WTW.getAsyncJSON('https://3dnet.roomz.com/connect/servicecheck.php?serverinstanceid=' + dGet('wtw_serverinstanceid').value + '&serverip=' + dGet('wtw_serverip').value + '&service=' + zservice + '&userid=' + dGet('wtw_tuserid').value, 
			function(zresponse) {
				zresponse = JSON.parse(zresponse);
...
WTW_3DINTERNET.prototype.openActivateWindow = function() {
	...
	try {
		WTW.openIFrame('https://3dnet.roomz.com/core/pages/serviceactivation.php?serverinstanceid=' + btoa(dGet('wtw_serverinstanceid').value) + '&domainname=' + btoa(wtw_domainname) + '&domainurl=' + btoa(wtw_domainurl) + '&websiteurl=' + btoa(wtw_websiteurl) + '&serverip=' + btoa(dGet('wtw_serverip').value) + '&userid=' + btoa(dGet('wtw_tuserid').value) + '&useremail=' + btoa(dGet('wtw_tuseremail').value), .6, .9, 'roomz Service Activation');
	} catch (ex) {
...
WTW_3DINTERNET.prototype.enableChat = async function(zchecked) {
	...
	try {
		...
		if (wtw3dinternet.masterChat == '1') {
			...
			wtw3dinternet.initChatSocket();
			WTW.getAsyncJSON('https://3dnet.roomz.com/connect/servicehold.php?serverinstanceid=' + dGet('wtw_serverinstanceid').value + '&serverip=' + dGet('wtw_serverip').value + '&service=chat&hold=0&userid=' + dGet('wtw_tuserid').value, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					wtw3dinternet.setActiveText(zresponse);
				}
			...
			/* set chat on hold */
			WTW.getAsyncJSON('https://3dnet.roomz.com/connect/servicehold.php?serverinstanceid=' + dGet('wtw_serverinstanceid').value + '&serverip=' + dGet('wtw_serverip').value + '&service=chat&hold=1&userid=' + dGet('wtw_tuserid').value, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					wtw3dinternet.setActiveText(zresponse);
				}
...
WTW_3DINTERNET.prototype.enableVoiceChat = async function(zchecked) {
	...
	try {
		...
		if (wtw3dinternet.masterVoiceChat == '1') {
			...
			wtw3dinternet.initVoiceChatSocket();
			WTW.getAsyncJSON('https://3dnet.roomz.com/connect/servicehold.php?serverinstanceid=' + dGet('wtw_serverinstanceid').value + '&serverip=' + dGet('wtw_serverip').value + '&service=voicechat&hold=0&userid=' + dGet('wtw_tuserid').value, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					wtw3dinternet.setActiveText(zresponse);
				}
			...
			WTW.getAsyncJSON('https://3dnet.roomz.com/connect/servicehold.php?serverinstanceid=' + dGet('wtw_serverinstanceid').value + '&serverip=' + dGet('wtw_serverip').value + '&service=voicechat&hold=1&userid=' + dGet('wtw_tuserid').value, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					wtw3dinternet.setActiveText(zresponse);
				}
...
WTW_3DINTERNET.prototype.enableDownloads = async function(zchecked) {
	/* enable roomz Downloads for 3D Webs options */
	try {
...
WTW_3DINTERNET.prototype.enablePlugins = async function(zchecked) {
	/* enable roomz Downloads for 3D Plugins options */
	try {
	...
	try {
		...
		switch (zitem) {
			case 'Global Local Profile':
				zpagediv += "<h2 class='wtw-login'>roomz Profile</h2>";
			...
			case 'Login Menu':
				...
				if (WTW.globalLogins == '1') {
					zpagediv += "<div class='wtw-loginbutton' onclick='wtw3dinternet.openGlobalLogin();'><img src='/content/system/images/menuwtw.png' alt='roomz' title='roomz' class='wtw-image40'/><img id='wtw_globalcheck' src='/content/system/images/greencheck.png' class='wtw-imageright40' /><div style='margin-top:4px;'>roomz Login<br /><span style='font-size:.6em;'>(Works on most roomz 3D Websites)</span></div></div>";
				}
				if (WTW.localLogins == '1') {
					zpagediv += "<div class='wtw-loginbutton' onclick=\"WTW.openLocalLogin('3D Website Login', .4, .6);\"><img src='/content/system/images/icon-128x128.jpg' alt='Bixma' title='Bixma' class='wtw-image40'/><img id='wtw_localcheck' src='/content/system/images/greencheck.png' class='wtw-imageright40' /><div style='margin-top:4px;'>3D Website Login<br /><span style='font-size:.6em;'>(3D Websites on this Server Only)</span></div></div>";
				}
				if (dGet('wtw_tuserid').value != '') {
					if (WTW.globalLogins == '1') {
						zpagediv += "<div class='wtw-logincancel' onclick='wtw3dinternet.logoutGlobal();'>Logout roomz</div>&nbsp;&nbsp;";
					}
					if (WTW.localLogins == '1') {
...
WTW_3DINTERNET.prototype.hudLoginLoadAvatarsArray = function(zfilter, zdefaultdisplayname) {
	...
	try {
		if (WTW.globalLogins == '1') {
			...
			WTW.postJSON('https://3dnet.roomz.com/connect/globalavatars.php', zrequest, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					...
					function(zresponse) {
						...
						if (zresponse.globalhash != undefined) {
							if (zresponse.globalhash != '') {
								...
								WTW.postAsyncJSON('https://3dnet.roomz.com/connect/globalquicksaveavatar.php', zrequest, 
									function(zresponse) {
										zresponse = JSON.parse(zresponse);
										/* note serror would contain errors */
...
WTW_3DINTERNET.prototype.downloadUserAvatarVersion = function(zobj, zglobaluseravatarid, zuseravatarid, zupdateuseravatarid, zwebid, zupdatewebid, zversionid, zversion, zoldversion, zwebtype) {
	...
	try {
		...
		if (zglobaluseravatarid != '') {
			WTW.postAsyncJSON('https://3dnet.roomz.com/connect/globalsaveavatar.php', zrequest, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
...
WTW_3DINTERNET.prototype.logoutGlobal = function() {
	/* references 3dnet.roomz.com - logout of global roomz login */
	try {
		WTW.openLoginHUD('roomz Login');
//		WTW.openIFrame('https://3dnet.roomz.com/core/login/login.php?logout=1&serverinstanceid=' + btoa(dGet('wtw_serverinstanceid').value) + '&domainname=' + btoa(wtw_domainname) + '&domainurl=' + btoa(wtw_domainurl) + '&websiteurl=' + btoa(wtw_websiteurl) + '&webid=' + btoa(communityid + buildingid + thingid), .4, .6, 'Login Menu');
	} catch (ex) {
...
WTW_3DINTERNET.prototype.openGlobalLogin = function() {
	/* opens login for 3dnet.roomz.com - as a global roomz login option */
	try {
		WTW.openLoginHUD('roomz Login');
//		WTW.openIFrame('https://3dnet.roomz.com/core/login/login.php?serverinstanceid=' + btoa(dGet('wtw_serverinstanceid').value) + '&domainname=' + btoa(wtw_domainname) + '&domainurl=' + btoa(wtw_domainurl) + '&websiteurl=' + btoa(wtw_websiteurl) + '&webid=' + btoa(communityid + buildingid + thingid), .4, .6, 'Login Menu');
	} catch (ex) {
...
WTW_3DINTERNET.prototype.onMessage = function(zevent) {
	/* message listener is enabled and this function can receive predefined messages from an iframe within the roomz instance - This function allows calls from 3dnet.roomz.com and .network */
	var zsafe = false;
	try {
		if (zevent.origin == 'https://3dnet.roomz.com') {
			zsafe = true;
		} else if (zevent.origin == 'https://3dnet.roomz.network') {
			zsafe = true;
		}
	} catch (ex) {
...
WTW_3DINTERNET.prototype.addConnectingGrid = function(zconnectinggridsurl, zchildwebtype, zchildwebid, zchildwebname, zfranchiseid, zserverfranchiseid, zwebalias, zparentname) {
	...
	try {
		if (zfranchiseid != '') {
			zconnectinggridsurl = 'https://3dnet.roomz.com/connect/franchiseconnectinggrids.php?franchiseid=' + zfranchiseid + '&serverfranchiseid=' + zserverfranchiseid + '&webalias=' + zwebalias + '&parentname=' + zparentname + '&startpositionx=0&startpositiony=0&startpositionz=0';
		}
	} catch (ex) {
...
WTW_3DINTERNET.prototype.addConnectingGridActionZones = function(zactionzonesurl, zchildwebtype, zchildwebid, zchildwebname, zfranchiseid, zserverfranchiseid, zwebalias, zparentname, zconnectinggridid, zconnectinggridind) {
	...
	try {
		if (zfranchiseid != '') {
			zactionzonesurl = 'https://3dnet.roomz.com/connect/franchiseactionzones.php?franchiseid=' + zfranchiseid + '&serverfranchiseid=' + zserverfranchiseid + '&webalias=' + zwebalias + '&parentname=' + zparentname + '&connectinggridid=' + zconnectinggridid + '&connectinggridind=' + zconnectinggridind;
		}
	} catch (ex) {
...
WTW_3DINTERNET.prototype.feedbackSubmit = function(zrequest) {
	/* forward feedback to roomz hub to be logged */
	try {
		WTW.postAsyncJSON('https://3dnet.roomz.com/connect/feedback.php', zrequest, 
			function(zresponse) {
				zresponse = JSON.parse(zresponse);
			}
		);
...
WTW_3DINTERNET.prototype.getSavedAvatar = function(zglobaluseravatarid, zinstanceid, zavatarname, zsendrefresh) {
	...
	try {
		if (zglobaluseravatarid != '') {
			/* global avatar - uses a secure post method to 3dnet.roomz.com */
			var zrequest = {
			...
			WTW.postAsyncJSON('https://3dnet.roomz.com/connect/globalavatar.php', zrequest, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
...
WTW_3DINTERNET.prototype.deleteUserAvatar = function(zglobaluseravatarid, zuseravatarid, zwidth, zheight) {
	...
	try {
		if (zglobaluseravatarid != '') {
			...
			/* send request to global server avatars handler */
			WTW.postAsyncJSON('https://3dnet.roomz.com/connect/globalsaveavatar.php', zrequest2, 
				function(zresponse2) {
