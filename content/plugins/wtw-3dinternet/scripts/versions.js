...
WTW_3DINTERNET.prototype.getPluginInfoComplete = async function(zmyplugins, zplugins, zshow, zfilter) {
	...
	try {
		...
		WTW.getAsyncJSON('https://3dnet.roomz.com/connect/checkforupdates.php?list=2&pluginnames=' + zplugins, 
						...
						if (zmyplugins[i].pluginname != undefined && zmyplugins[i].version != undefined && zmyplugins[i].latestversion != undefined) {
							if (zmyplugins[i].pluginname == 'roomz') {
								...
								if (zmyplugins[i].latestversion == wtw_version) {
									zupdateslist += "<div class='wtw-dashboardboxtitle'>roomz is up to date!</div><div class='wtw-dashboardbox'><b>Your Version:</b><hr />";
									...
									zupdateslist += "<div class='wtw-dashboardboxtitle'>roomz has an update!</div><div class='wtw-dashboardbox'>Your Version: " + wtw_version + " (" + zversiondatestring + ")<br /><br />";
								...
								if (zmyplugins[i].latestversion != wtw_version && WTW.isUserInRole('Admin')) {
									zupdateslist += "<div class='wtw-greenmenubutton' onclick=\"WTW.updateroomz('" + zmyplugins[i].pluginname + "','" + zmyplugins[i].latestversion + "','" + zdatestring + "','" + zmyplugins[i].updateurl + "');\">Update Now!</div>";
							...
							function(zresponse3) {
								...
								WTW.getAsyncJSON('/connect/avatars.php', 
									function(zresponse4) {
										...
										WTW.postAsyncJSON('https://3dnet.roomz.com/connect/versioncheck.php', zrequest, 
											function(zresponse5) {
...
WTW_3DINTERNET.prototype.getPluginLatestVersion = async function(zpluginname) {
	var zversion = '';
	try {
		WTW.getAsyncJSON('https://3dnet.roomz.com/connect/versioncheck.php?pluginname=' + zpluginname, 
			function(zresponse) {
...
WTW_3DINTERNET.prototype.loadArchiveUpdates = async function() {
	/* get all archive version details for roomz */
	try {
		dGet('wtw_archiveupdateslist').innerHTML = '';
		WTW.getAsyncJSON('https://3dnet.roomz.com/connect/wtwupdates.php', 
			function(zresponse) {
				zresponse = JSON.parse(zresponse);
				if (zresponse[0] != null) {
					var zupdateid = '';
					...
					zarchiveupdateslist += "<div class='wtw-dashboardboxtitle'>Archive - roomz Update Details</div><div class='wtw-dashboardbox'>";
					for (var i=0; i < zresponse.length;i++) {
						if (zresponse[i] != null) {
							...
							if (zresponse[i].imageurl != '') {
								zimage = "<img src='" + zresponse[i].imageurl + "' title='" + zresponse[i].updatetitle + "' alt='" + zresponse[i].updatetitle + "' style='width:120px;height:auto;float:left;margin:8px 18px 8px 0px;cursor:pointer;' onclick=\"WTW.openIFrame('/core/pages/imageviewer.php?imageurl=" + zresponse[i].imageurl + "', .8, .8, 'roomz Update Image');\" />";
							}
...
WTW_3DINTERNET.prototype.getVersionDetails = async function(zupdateid) {
	...
	try {
		...
		WTW.getAsyncJSON('https://3dnet.roomz.com/connect/versiondetails.php?updateid=' + zupdateid, 
			function(zresponse) {
				zresponse = JSON.parse(zresponse);
				if (zresponse[0] != null) {
					var zupdatedetailslist = "<div class='wtw-dashboardboxleftfull'>";
					zupdatedetailslist += "<div class='wtw-dashboardboxtitle'>roomz Update <b>" + zresponse[0].version + "</b> Details</div><div class='wtw-dashboardbox'>Update Details: <br /><ul>";
					for (var i=0; i < zresponse.length;i++) {
						if (zresponse[i] != null) {
...
WTW_3DINTERNET.prototype.showListVersionCheck = function(zwebtype, zversioncheck) {
	...
	try {
		...
		WTW.postAsyncJSON('https://3dnet.roomz.com/connect/versioncheck.php', zrequest2, 
			function(zresponse2) {
