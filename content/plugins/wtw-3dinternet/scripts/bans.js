...
WTW_3DINTERNET.prototype.banUserInstance = function(zbaninstanceid) {
	...
	try {
		...
		if (zbanavatar != null) {
			...
			/* report to roomz to assist in predicting bans globally */
			WTW.postAsyncJSON('https://3dnet.roomz.com/connect/wtwsaveban.php', zrequest, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					/* note serror would contain errors */
					wtw3dinternet.avatarConnectMenu(zbaninstanceid, true);
...
WTW_3DINTERNET.prototype.blockChat = function(zbaninstanceid) {
	...
	try {
		...
		if (zbanavatar != null) {
			...
			/* report block or ban to roomz to assist in predicting bans globally */
			WTW.postAsyncJSON('https://3dnet.roomz.com/connect/wtwsaveban.php', zrequest, 
				function(zresponse) {
					zresponse = JSON.parse(zresponse);
					/* note serror would contain errors */
